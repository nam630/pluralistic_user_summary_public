...
def preprocess_data(data, input_template=None, input_key="input", output_key=None, apply_chat_template=None, multiturn=False, custom_system_message=None):
    if apply_chat_template:
        ...
        print("Using this system message: ", custom_system_message)
        if output_key:
            ...
            if custom_system_message is not None:
                prompt = apply_chat_template([{"role": "system", "content": custom_system_message}] + prompt_message, tokenize=False, add_generation_prompt=True)
                response = apply_chat_template([{"role": "system", "content": custom_system_message}] + prompt_message + response_message, tokenize=False)[len(prompt) :]
            else:
            ...
            if custom_system_message is not None:
                prompt = apply_chat_template([{"role": "system", "content": custom_system_message}] + data[input_key][:-1], tokenize=False, add_generation_prompt=True)
                response = apply_chat_template([{"role": "system", "content": custom_system_message}] + data[input_key], tokenize=False)[len(prompt) :]    
            else:
...
class SFTDataset(Dataset):
    ...
    def __init__(
        ...
        print("IS multi turn training?", self.multiturn)

        # chat template
        self.input_template = input_template
        self.input_key = getattr(self.strategy.args, "input_key", None)
        self.output_key = getattr(self.strategy.args, "output_key", None)
        self.custom_system_message = getattr(self.strategy.args, "custom_system_message", None)
        self.apply_chat_template = getattr(self.strategy.args, "apply_chat_template", False)
        print(self.input_key, self.output_key, "\n\n")
        ...
        if self.multiturn:
            ...
            for idx, message in enumerate(data[input_key]):
                if message['role'] == 'assistant':
                    if idx == 0: # cannot start with assistant
                        continue
