...
class PPOEvaluator(ABC):
    ...
    def __init__(
        ...
        **generate_kwargs,
        ...
        print(f"***Currently specifying reward tokenizer to be {self.args.reward_pretrain} in PPO evaluator***")
        ...
        self.experience_maker = PersonalizedExperienceMaker(
            ...
            remote_rm_url,
            reward_fn,
            False # train_reward is set to False during eval
        )
    ...
    def eval(
        ...
        for rand_prompts, rejected, chosen, reward_prompts, train_prompts, eval_prompts, train_rejected, train_chosen, eval_rejected, eval_chosen in self.eval_dataloader:
            # number of for loops == rollout_batch_size // micro_batch_size
            for i, experience in enumerate(
                ...
                concatenated_summaries = experience.info["summary"]
                concatenated_eval_prompts = experience.info["eval_prompt"]
                concatenated_eval_rejected = experience.info["eval_rejected"]
                concatenated_eval_chosen = experience.info["eval_chosen"]
                self.experience_maker.evaluate_reward_model(concatenated_summaries, concatenated_eval_prompts, concatenated_eval_rejected, concatenated_eval_chosen, return_all=True)
                print(concatenated_summaries[-1],"\nRolling Eval ACC: ", np.mean(self.experience_maker.fin_eval_accs), "\nEval COUNT: ", len(self.experience_maker.fin_eval_accs))
                summaries.append(concatenated_summaries)

        save_path = os.path.join("results/" + "post_train_" + self.debug_log_path)
        acc_list = self.experience_maker.fin_eval_accs 
        rew_list = self.experience_maker.fin_eval_rews
        pickle.dump({"eval_acc": acc_list,  "eval_rew": rew_list, "eval_summary": summaries}, open(save_path, "wb"))
        print("\n\nRM eval size: ", len(acc_list), "RM eval acc: ", np.mean(acc_list), "\n", "RM eval rew:", np.mean(rew_list), "\n\n")
