...
from .ppo_utils import AdaptiveKLController, Experience, FixedKLController, NaiveExperienceMaker, NaiveReplayBuffer, PersonalizedExperienceMaker, PersonalizedExperienceEvaluator
...
class InferenceEvaluator(ABC):
    ...
    def __init__(
        ...
        **generate_kwargs,
        ...
        print("***Currently specifying reward tokenizer to be Qwen2.5-0.5B in PPO trainer***")
        self.reward_tokenizer = AutoTokenizer.from_pretrained(self.args.reward_pretrain, use_fast=True)
        self.experience_maker = PersonalizedExperienceEvaluator(
            actor,
            critic,
            reward_model,
            initial_model,
    ...
    def eval(
        ...
        for prompts, summaries, responses, p_responses in self.eval_dataloader:
            # number of for loops == rollout_batch_size // micro_batch_size
            rew, p_rew = self.experience_maker.evaluate_reward_model(summaries, prompts, responses, p_responses, return_all=True)
            default_rew.append(rew.item())
            personalized_rew.append(p_rew.item())
        print("default response: ", np.mean(default_rew), "\npersonalized response: ", np.mean(personalized_rew))
