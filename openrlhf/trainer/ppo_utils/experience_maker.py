...
class NaiveExperienceMaker(ABC):
    ...
    def make_experience_list(
            self, all_prompts: Union[str, List[str]], all_labels: Union[str, List[str]], **generate_kwargs
    ) -> List[Experience]:
        """
        ...
        torch.cuda.synchronize()
        print("L 238")
        # train the reward models with the samples here?
...
class ExperienceMaker(ABC):
    ...
    def make_experience(self, samples: Samples) -> Experience:
        ...
        if self.remote_rm_url is not None:
            ...
            # local RM (some process is taking forever to respond? check if it's just running time issue?)
            # (Expects this line to be reached!)
            # print("Entered line 374 this is reached ....\n")
            # print("Prompts: ", samples.prompts, "\n\n")
            # print("Sequences: ", samples.sequences, "\n\n")
            # print("Labels: ", samples.labels, "\n\n")
            r = self.reward_model(sequences, attention_mask)
            if len(r.shape) > 1:
                if self.single_train == "sentiment":
                    print("Using only sentiment as a combination of the positive and negative rews!")
                    weights = torch.tensor([0, 1., -1.], device=r.device)
                elif self.single_train == "repetition":
                    print("Using only repetition as negative reward!")
                    weights = torch.tensor([-1, 0., 0.], device=r.device)
                else:
                    print("Using all three -- repetition, positive, negative sentiments!")
                    weights = torch.tensor([-0.5, 1., -1.], device=r.device)
                r = (r * weights).sum(dim=1) 
            # print(r.shape, "Reward output shape in L810") # [2,3] batch size, 3 dimensiosn --> need to collapse into a scalar for training
